...
async function getRegisteredUserFromField(field:string, value:string){
    try {
        const [results]:any = await con.query(`SELECT * FROM Users WHERE ${field}='${value}' LIMIT 1;`)
        if (results.length === 0){
            return null
        }
...
async function registerNewUser(newUser: {uuid: string; email: string; username: string; password: string} ) {
    try {
...
async function loginIsValid(loginAttempt:{email:string; username:string; password:string;}){
    return true
}
...
async function insertNewClientId(user: { id: number; }, clientId: string, secret: string) {
    try {
...
function validateInput(input: { username: string; email: string; password: string; passwordRepeated:string; uuid: string; }) {
    const userInput : { valid: boolean; errors: string[];} = {
        "valid" : true,
        "errors" : []
    }
